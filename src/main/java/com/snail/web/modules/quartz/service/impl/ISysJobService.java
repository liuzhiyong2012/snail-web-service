...
import com.snail.web.modules.quartz.mapper.SysJobMapper;
import com.snail.web.modules.quartz.service.SysJobService;
import com.snail.web.modules.quartz.utils.CronUtils;
import com.snail.web.modules.quartz.utils.ScheduleUtils;
...
@Service
public class ISysJobService extends ServiceImpl<SysJobMapper, SysJob> implements SysJobService {
    @Autowired
    ...
    @Override
    public PageBaseResponse page(SysJob sysJob, String userId) {
        if (sysJob.getPageNumber() == null || sysJob.getPageSize() == null) {
            return ResponseUtils.pageError("参数缺失分页");
        }
        Integer count = this.baseMapper.count(sysJob);
        if (count == null || count.equals(0)) {
            return ResponseUtils.pageSuccess();
        }
        List<SysJob> reponses = this.baseMapper.page(sysJob);

        return ResponseUtils.pageConvert(sysJob.getPageNumber(), sysJob.getPageSize(), count, reponses);
    }
